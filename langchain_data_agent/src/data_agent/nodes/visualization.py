...
class VisualizationNode:
    ...
    async def generate_visualization(self, state: "AgentState") -> dict[str, Any]:
        ...
        """
        # Clear any previous visualization to prevent showing stale data
        updates = {"visualization_image": None, "visualization_error": None}
        
        result = state.get("result")
        if not result:
            logger.warning("Visualization requested but no query result available")
            updates["visualization_error"] = "No data to visualize"
            return updates
...
"""
        ...
        try:
            ...
            if not code:
                ...
                updates["visualization_error"] = "Failed to generate visualization code"
                return updates

            # Prepend the data variable to the code so it's available during execution
            full_code = f"data = {data}\n\n{code}"
            
            logger.debug("Generated visualization code:\n%s", code[:500])

            # Execute code in sandbox
            exec_result = await self._executor.execute(full_code)
            ...
            if exec_result.status == ExecutionStatus.SUCCESS:
                ...
                updates["visualization_error"] = f"Code executed but no image: {exec_result.output[:200]}"
                return updates
            logger.error("Code execution failed: %s", exec_result.error)
            updates["visualization_error"] = exec_result.error
            return updates

        except Exception as e:
            logger.exception("Visualization code execution failed")
            updates["visualization_error"] = str(e)
            return updates
