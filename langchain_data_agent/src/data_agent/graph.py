...
class DataAgentGraph:
    ...
    def _route_after_execute(self, state: AgentState) -> str:
        ...
        if state.get("error"):
            return "error"
        # Only route to visualization if executor is available (not LocalExecutor fallback)
        if state.get("visualization_requested", False) and self._viz_node._executor is not None:
            return "visualize"
        return "respond"
