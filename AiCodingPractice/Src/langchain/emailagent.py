...
# Export the agent graph for LangGraph deployment
graph = agent
...
if __name__ == "__main__":
    ...
    async def main():
        print("=== Email Agent with Authentication & Human-in-the-Loop ===\n")
        
        print("Step 1: Agent detects you're not authenticated and will try to authenticate")
        ...
        print("Step 2: After authentication, checking inbox")
        response = await agent.ainvoke(
            {
                "messages": [HumanMessage(content="Show me my emails")],
                "authenticated": True
            },
            context=EmailContext()
        )
