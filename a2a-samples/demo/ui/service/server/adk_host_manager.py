...
class ADKHostManager(ApplicationManager):
    ...
    def __init__(
        ...
        self.app_name = 'A2A'
        # Check for Azure OpenAI first, then Google API key
        self.api_key = api_key or os.environ.get('AZURE_API_KEY', '') or os.environ.get('GOOGLE_API_KEY', '')
        self.uses_vertex_ai = (
            uses_vertex_ai
            or os.environ.get('GOOGLE_GENAI_USE_VERTEXAI', '').upper() == 'TRUE'
        )
        self.uses_azure = bool(os.environ.get('AZURE_API_KEY', ''))

        # Set environment variables based on auth method
        if self.uses_azure:
            # Azure OpenAI configuration via LiteLLM
            os.environ['AZURE_API_KEY'] = self.api_key
        elif self.uses_vertex_ai:
